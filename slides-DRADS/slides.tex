...
\date{May 3\textsuperscript{rd}, 2018 \\[1em] DRADS 2018, Leuven}
...
\end{Coq}
\begin{align*}
\mathtt{Vec} = & \mathsf{Ind}(X: \text{\Coqe{nat}} \to \text{\Coqe{Type}})\\
               & \{\text{\Coqe{nil}} : X~0,
                 \text{\Coqe{cons}} : \Pi n : \text{\Coqe{nat}}.\; T \to X~n \to X~(S~n)\}
\end{align*}
\[ T : \text{\Coqe{Type}} \vdash \mathtt{Vec} : \text{\Coqe{nat}} \to \text{\Coqe{Type}} \]
\end{itemize}
\end{frame}
